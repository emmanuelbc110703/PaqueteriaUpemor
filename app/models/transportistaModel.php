...
//crer clase del modelo
    ...
    class transportistaModel{
        ...
        public function insertartransportista($nombre,$edad,$telefono,$correo,$matricula,$tipolicencia,$ultimafecha){
            //crear statemente

            $sql_statemente = "INSERT INTO transportista(nombre,edad,telefono,correo,matricula,tipolicencia,ultimafecha) VALUES (?,?,?,?,?,?,?)";
            
            //preparar el statement
            $statement = $this -> connection -> prepare($sql_statemente);
            $statement -> bind_param("sisssss", $nombre,$edad,$telefono,$correo,$matricula,$tipolicencia,$ultimafecha);
        ...
        public function consultartransportista(){
            ...
            $sql_statemente =  "SELECT * FROM transportista";
        ...
        public function consultarPorID($id_browser){
            ...
            $sql_statement = "SELECT * FROM  transportista WHERE id_conductor = ?";
        ...
        public function actualizartransportista($id_conductor,$nombre,$edad,$telefono,$correo,$matricula,$tipolicencia,$ultimafecha) {
            $sql_statement = "UPDATE transportista SET nombre = ?, edad = ?, telefono = ?, correo = ?, matricula = ?, tipolicencia = ?, ultimafecha = ? WHERE id_conductor = ?"; 

            $statement = $this -> connection -> prepare($sql_statement); 
            $statement -> bind_param("sisssssi", $nombre,$edad,$telefono,$correo,$matricula,$tipolicencia,$ultimafecha,$id_conductor);
            
            return $statement -> execute(); 
        }

        public function eliminartransportista($id){
            $sql_statement = "DELETE FROM transportista WHERE id_conductor = ?";
            $statement = $this -> connection -> prepare($sql_statement); 
